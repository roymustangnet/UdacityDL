...
def get_init_cell(batch_size, rnn_size):
    ...
    initial_state = tf.identity(initial_state, name = 'initial_state') # 提交作业后需要再查一下
...
def get_batches(int_text, batch_size, seq_length):
    ...
    # TODO: Implement Function
    # 一个batch中有多少元素
    one_batch_num = batch_size * seq_length
    # 总共有多少个batch #7
    total_batches = len(int_text) // one_batch_num
    
    # 输入
    x_data = np.array(int_text[0:one_batch_num*total_batches])
    # 输出
    y_data = np.array(int_text[1:one_batch_num*total_batches+1])
    ...
    for i in range(total_batches):
        idx = i * seq_length
        result.append([x_data[:,idx:idx+seq_length], 
                       y_data[:,idx:idx+seq_length]])
...
# > **疑问** 为什么使用2层以上的LSTM进行堆叠后，损失的降低速度反倒有所下降？
...
# > np.random.choice 可以从一个数组中以一定的概率提取若干值
